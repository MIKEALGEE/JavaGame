...
public class Camera {
    ...
    public void ssetLimit(int widthLimit, int heightLimit){
    ...
    public void update() {
        move();
        if (!e.xCol) {
            if (e.getBounds().getPos().getWorldVar().x + e.getDx() < Vector2f.getWorldVarX(widthLimit - GamePanel.width / 2) - 64 && (e.getBounds().getPos().getWorldVar().x + e.getDx()) > Vector2f.getWorldVarX(widthLimit -collisionCam.getWidth() / 2 - 64)) {
                PlayState.map.x += dx;
                bounds.getPos().x += dx;
            }
        }
        if (!e.yCol) {
            if (e.getBounds().getPos().getWorldVar().y + e.getDy() < Vector2f.getWorldVarY(heightLimit - GamePanel.height / 2) - 64 && (e.getBounds().getPos().getWorldVar().y + e.getDy()) > Vector2f.getWorldVarY(heightLimit- collisionCam.getHeight() / 2 - 64)) {
                PlayState.map.y += dy;
                bounds.getPos().y += dy;
            }
        }
    ...
    public void input(MouseHandler mouse, KeyHandler key){
        ...
        if (e == null) {
            ...
            if(!e.xCol){
                if(PlayState.map.x + GamePanel.height /2 - e.getSize() / 2 + dx > e.getBounds().getPos().x + e.getDx() + 2){
                    left = true;
                    right = false;
